...
const AdminBookingManagement = () => {
  ...
  const filterBookings = () => {
    ...
    switch (activeTab) {
      case 'new':
        filtered = filtered.filter(booking => booking.status?.toLowerCase() === 'confirmed' && !booking.isViewed);
        break;
      case 'upcoming':
        filtered = filtered.filter(booking => {
          const checkIn = parseISO(booking.checkIn);
          return isAfter(checkIn, today) && booking.status?.toLowerCase() === 'confirmed';
        });
        break;
      case 'today':
        filtered = filtered.filter(booking => {
      ...
      case 'completed':
        filtered = filtered.filter(booking => {
          const checkOut = parseISO(booking.checkOut);
          return isBefore(checkOut, today) && booking.status?.toLowerCase() === 'confirmed';
        });
        break;
      default: // 'all'
        break;
